...
router.post("/new", async(request, response) => {
    ...
    try {
        ...
        if(!post) return response.status(500).send({errorMessage: "Error while creating post"})
        response.send({message: "Successfully created post", post})
    } catch (e) {
        response.status(500).send({errorMessage: "Internal error", error: e})
    }
})
...
router.get('/all/:userId', async (request, response) => {
    ...
    try{
        const post = await Post.find({author: userId}).populate('author')
        response.json(post)
    }
    catch(e) {
